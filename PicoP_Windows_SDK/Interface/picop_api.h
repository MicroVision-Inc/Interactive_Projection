...
//                                     INPUT CONTROL
// ************************************************************************************************
// Note: The functions parameters will change based on what is needed for
// MIPI-DSI controls
...
//! \brief Perform an Electronics Package firmware update.
//!
//! \param connectionHandle: A valid connection handle previously obtained by a
//!            call to PicoP_OpenConnection().
//! \param byteTotal: Size of the firmware image in total bytes.
//! \param image: Pointer to a byte array containing a firmware upgrade image.
//!
PicoP_ReturnStatus PicoP_UpgradeSoftwareEx(PicoP_HANDLE *connectionHandle,
                                             uint32_t byteTotal,
                                             uint8_t const *image);
...
//!
//!        If the EPM is connected to an input video source, the test pattern
//!        will not display. Use PicoP_SetInputVideoState() to disable input
//!        prior to using PicoP_DrawTestPattern().
//!
//! \param connectionHandle: A valid connection handle previously obtained by a
//!            call to OpenConnection().
//!
//! \param testPattern: Test pattern to draw. Supported Test patterns:
//!            eCHECKERBOARD_PATTERN
//!            eCROSSHAIR_PATTERN
//!            eGRID_PATTERN
//!            eCONSTANT_COLOR_PATTERN
//!
//! \param foregroundColor: 24-Bit RGB foreground pattern color value in a
//!            0xRRGGBB hex format.
//!
//! \param backgroundColor: 24-Bit RGB background color value in a 0xRRGGBB hex
//!            format.
//!
